...
public sealed class OtpService
{
    ...
    /// <see cref="IdentityErrors.OtpRateLimited"/> if the cooldown has not elapsed.
    /// </exception>
        ...
        if (existing is not null)
        {
            ...
            if (elapsed < _options.ResendCooldown)
            {
                ...
                var rateLimited = new IdentityException(
                    IdentityErrors.OtpRateLimited, 429, "A code was sent recently.");
    ...
    /// </remarks>
    /// <exception cref="IdentityException">
    /// <see cref="IdentityErrors.OtpExpired"/>, <see cref="IdentityErrors.OtpInvalid"/>
    /// or <see cref="IdentityErrors.OtpLocked"/>.
    /// </exception>
        ...
        OtpPurpose purpose, string destination, string code, CancellationToken ct = default)
    {
        ...
        if (now >= challenge.ExpiresAt)
        {
            ...
            throw new IdentityException(IdentityErrors.OtpExpired, 422, "The code has expired.");
        }
        ...
        if (left <= 0)
        {
            await _store.RemoveAsync(purpose, destination, ct).ConfigureAwait(false);
            throw new IdentityException(
                IdentityErrors.OtpLocked, 429, "Too many wrong codes. Request a new one.");
        }

        await _store.SaveAsync(challenge with { AttemptsLeft = left }, ct).ConfigureAwait(false);

        var wrong = new IdentityException(IdentityErrors.OtpInvalid, 422, "Wrong code.");
