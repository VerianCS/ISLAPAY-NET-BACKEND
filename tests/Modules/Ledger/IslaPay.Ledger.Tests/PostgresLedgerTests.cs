...
public class PostgresLedgerTests
{
    ...
    public async Task Opening_accounts_is_idempotent()
    {
        ...
        await ledger.EnsureUserAccountsAsync(user, [Currency.Usd, Currency.Usdt]);
    ...
    public async Task A_new_account_shows_zero_rather_than_nothing()
    {
        ...
        await ledger.EnsureUserAccountsAsync(user, [Currency.Usd]);
        var balances = await ledger.BalancesAsync(user);

        // A wallet that hides a zero balance looks broken to someone who has
        // just signed up.
        Assert.Equal(Currency.Usd, Assert.Single(balances).Currency);
    }
    ...
    public async Task A_posting_moves_money_and_still_sums_to_zero()
    {
        ...
        Assert.Equal(0, await SumOfEveryEntryAsync(Currency.Usd));
    }
    ...
    private static Posting Deposit(string user, string amount, string? key = null) =>
        new(
            ...
            [
                new Leg(AccountId.User(user, Currency.Usd), Money.Parse(amount, Currency.Usd)),
                new Leg(AccountId.CashFloat(Currency.Usd), Money.Parse($"-{amount}", Currency.Usd)),
            ],
