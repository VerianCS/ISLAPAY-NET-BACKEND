...
/// <summary>
/// A throwaway database with the custody schema applied.
/// </summary>
/// <remarks>
...
/// which carry the rule this module exists for: nothing is credited before
/// finality, and the database refuses a row that claims otherwise.
/// </remarks>
public sealed class CustodyFixture : PostgresFixture
{
    protected override Task AfterCreateAsync() => Migrator.ApplyAsync(Database,
    [
        new MigrationSet("custody", typeof(CustodyModule).Assembly, "IslaPay.Custody.Migrations."),
    ]);

    public CustodyDbContext Context()
    {
        var builder = new DbContextOptionsBuilder<CustodyDbContext>();
        builder.UseNpgsql(Options.ConnectionString);
        return new CustodyDbContext(builder.Options);
    }
    ...
    /// whole table by design, so a deposit one test left behind is one the
    /// next test's sweeper acts on — against that test's own fake ledger,
    /// where it shows up as somebody else's money moving.
    /// </remarks>
    ...
    public CustodyService Service(
        ...
        IDepositAddresses? addresses = null,
...
public sealed class CustodyDefinition : ICollectionFixture<CustodyFixture>
{
    public const string Name = "custody-schema";
}
